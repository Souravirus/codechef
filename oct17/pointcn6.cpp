...
int main() {
	...
	for (int t = 0; t < T; t++) {
        ...
        dout << "printing Carr:" << endl;
        for(ulli i = 0; i < n; i++){
            dout << "for " << Carr[i].second <<":" <<  Carr[i].first << endl;
        }
        ...
        while(tCount < n){
            ...
            for(ulli j = 0; j < n; j++){
                ...
                if((maini[i][A[j].second] == 0 && i != A[j].second)){
					if(traversed[A[j].second] == 0){
                        ...
                        if(C[i][A[j].second] > maxEdge[A[j].second] || Ccount[A[j].second] + 1 > recommended[A[j].second])
            ...
            if(flag == 1){
                ...
                while(tcount < minCount && j < n){
                    ...
                    if((maini[i][A[j].second] == 0 && i != A[j].second && !obscured[A[j].second])){
