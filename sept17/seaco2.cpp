...
{
  seg_tree[node_num] = (seg_tree[node_num]+(lazy[node_num]%mod))%mod;
  if(e>s)
  {
   lazy[node_num*2+1] = (lazy[node_num*2+1]+(lazy[node_num]%mod));
   lazy[node_num*2] = (lazy[node_num*2]+(lazy[node_num]%mod)); 
  }
   lazy[node_num] = 0;
}
...
void set_lazy(lli s,lli e,lli new_val,lli node_num, lli *lazy)
{
   ...
   {
     lli m = (s+e)/2; 
    lazy[node_num*2+1] = (lazy[node_num*2+1]+(new_val%mod));
     lazy[node_num*2]= (lazy[node_num*2]+(new_val%mod));
   }
}
...
{ 
  ...
  {
    seg_tree[node_num]=(seg_tree[node_num]+(new_val%mod))%mod;
    ...
    while(T--)
    {
        ...
        for(lli i=m; i>=1; i--)
        {
           ...
           //cout<<"for "<<i<<" upd "<<upd<<endl;
            if(type[i]==2)
            {
               	range_update(start[i], end[i], (upd+1)%mod, 1, m, 1, seg_tree, lazy); 
            }
        }
        ...
        for(lli i=1; i<=m; i++)
        {
            ...
            {
                lli upd=query( i, i, 1, m, 1, seg_tree, lazy);
               	range_update(start[i], end[i], (upd+1)%mod, 1, n, 1, seg_tree2, lazy_arr); 
            }
        }
        for(lli i=1; i<=n; i++)
        {
            lli upd=query( i, i, 1, m, 1, seg_tree2, lazy_arr);
            cout<<upd%mod<<" ";
        }
        cout<<endl;
    }
